...
int	rotate_both_forward(t_stack **a, t_stack **b, int target, char stack_type)
{
    if (stack_type == 'a')
    {
        while ((*a)->nbr != target && determine_position_in_stack_b(*b, target) > 0)
            rr(a, b, 0);
        while ((*a)->nbr != target)
            ra(a, 0);
        while (determine_position_in_stack_b(*b, target) > 0)
            rb(b, 0);
        pb(a, b, 0);
    }
    else
    {
        while ((*b)->nbr != target && determine_position_in_stack_a(*a, target) > 0)
            rr(a, b, 0);
        while ((*b)->nbr != target)
            rb(b, 0);
        while (determine_position_in_stack_a(*a, target) > 0)
            ra(a, 0);
        pa(a, b, 0);
    }
    return (-1);
